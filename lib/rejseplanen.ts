    ...
    private buildRouteList(origin: string, destination: string, trips: Trip[]): RouteList {
        return <RouteList> {
            ...
            routes: trips.map((trip) => {
                return <Route>{
                    summary: buildTripSummaryMessage(trip),
                    details: buildTripDetailsMessage(trip)
                };
            })
        }
    }
...
export function buildTripDetailsMessage(trip: Trip): string {
    ...
    trip.Leg.forEach((leg) => {
        let startTime = parseDateTime(leg.Origin.date, leg.Origin.time);
        let endTime = parseDateTime(leg.Destination.date, leg.Destination.time);
        let legTime = minutesBetweenDates(endTime, startTime);
...
function buildTripInfo(trip: Trip): TripInfo {
    let tripInfo: TripInfo = <TripInfo>{
        startTime: parseDateTime(trip.Leg[0].Origin.date, trip.Leg[0].Origin.time),
        endTime: parseDateTime(trip.Leg[trip.Leg.length -1].Destination.date, trip.Leg[trip.Leg.length -1].Destination.time),
        totalTime: 0
    }
    tripInfo.totalTime = minutesBetweenDates(tripInfo.startTime, tripInfo.endTime); 
    return tripInfo;
}
...
function minutesBetweenDates(date1, date2): number {
